...
import static org.oclc.loganalyzer.ReportConsolidator.callAnalyzers;
...
public class ReportConsolidatorServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(hostNames!=null) {
                AccessLogAnalyzer.hostNames=new Properties();
                AccessLogAnalyzer.propertiesFile=new File(getServletContext().getRealPath(hostNames));
                AccessLogAnalyzer.closed=false;
                AccessLogAnalyzer.hostNames.load(new FileReader(AccessLogAnalyzer.propertiesFile));
            }
            String equivalents=request.getParameter("ipEquivalents");
            if(equivalents!=null)
                AccessLogAnalyzer.loadEquivalentAddresses(new FileReader(getServletContext().getRealPath(equivalents)));
            String spiders=request.getParameter("spiders");
            if(spiders!=null)
                AccessLogAnalyzer.loadSpiders(new FileReader(getServletContext().getRealPath(spiders)));
